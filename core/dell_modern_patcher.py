...
class DellModernPasswordPatcher:
    def __init__(self, file_path):
        ...
        # Dictionary of known UEFI Security/Password signatures
        self.security_signatures = {
            "AMITSESetup_Block": b'A\x00M\x00I\x00T\x00S\x00E\x00S\x00e\x00t\x00u\x00p',
            # Add specific hex patterns found via HxD for 8FC8/E7A8 below
        }
    ...
    def _is_payload_empty(self, payload_data):
        """Checks if the data block is completely empty (all 0xFF or all 0x00)."""
    ...
    def scan_file(self):
        ...
        for name, pattern in self.security_signatures.items():
            ...
            for offset in matches:
                ...
                # Assuming payload starts 16 bytes after the signature
        ...
        if not found_blocks:
            return {
                "status": "clean", 
                "message": "No active password flags detected. The file is already clean."
            }

        return {
            "status": "locked",
            "message": f"Active password data found in {len(found_blocks)} location(s).",
            "blocks": found_blocks
        }
    ...
    def _save_file(self):
        ...
        try:
            ...
            return True, f"Success! Password cleared. File saved as:\n{output_file}"
        except Exception as e:
            return False, f"Error saving file: {str(e)}"

    def auto_scan_and_remove(self):
        """
        Single function for GUI execution. Scans first, stops if clean, patches if locked.
        """
        if not self.is_loaded:
